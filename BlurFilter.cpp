...
BlurFilter::BlurFilter(ID3D12Device* device, UINT Width, UINT Height, DXGI_FORMAT Format): md3dDevice(device),mWidth(Width),mHeight(Height),mFormat(Format)
{
	BuildResources();
}
...
void BlurFilter::BuildResources()
{
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),D3D12_HEAP_FLAG_NONE,
													&texDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&mBlurMap0)));

	ThrowIfFailed(md3dDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		&texDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&mBlurMap1)));
...
void BlurFilter::Execute(ID3D12GraphicsCommandList* cmdList, ID3D12RootSignature* RootSig, ID3D12PipelineState* horzBlurPso, ID3D12PipelineState* verBlurPso, ID3D12Resource* input, int blurCount)
{
	...
	// Copy the input (back buffer in this case) to the mBlurMap0 texture
	...
	for (int i = 0; i < blurCount; ++i)
	{
		...
		//how many thread group do we need to dispatch to cover a row of pixels, where
			// each group covers 256 pixels (the 256 is defined in compute shader).
		...
		//how many thread group do we need to dispatch to cover a column of pixels, where
			// each group covers 256 pixels (the 256 is defined in compute shader).
	...
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(input, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mBlurMap0.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COPY_SOURCE));

	// mBlurMap0 back to back buffer
	cmdList->CopyResource(input,mBlurMap0.Get());


	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(input, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_RENDER_TARGET));
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mBlurMap0.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COMMON));
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mBlurMap1.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON));
}
...
std::vector<float> BlurFilter::CalcGaussWeights(float sigma)
{
	...
	// For example, for sigma = 2.5, the width of the bell curve is 5
	int blurRadius = (int)ceil(2.0f * sigma);

	assert(blurRadius <= MaxBlurRadius);

	std::vector<float> weights;
	// force size to be odd number so we always get a weight in the middle for the middle texel
