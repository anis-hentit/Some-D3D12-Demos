...
void HorzBlurCS(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	// this statement takes care of the threads that are not gonna load two texels ( so threads between gBlurRadius and N-gBlurRadius). it also
		// clamps out of bound samples that occur at image borders ( more precisely the min statement takes care of
	...
	int3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	// this statement takes care of the threads that are not gonna load two texels ( so threads between gBlurRadius and N-gBlurRadius). it also
	// Clamps out of bound samples that occur at image borders.( more precisely the min statement takes care of
		// cases where we have threads that are out of texture size but their groupThreadID.y doesnt have to be >= gBlurRadius (it can be >= but it can also be < gBlurRadius))
